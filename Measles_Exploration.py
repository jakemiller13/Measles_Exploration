...
# Show unique regions and countires for reference
regions, regions_counts = np.unique(df['Region'], return_counts = True)
print('\nRegions:\n' + str(regions))
countries, countries_counts = np.unique(df['Country'], return_counts = True)
print('\nCountries:\n' + str(countries))
...
def compare_locations(comp_dict, plot_all_years = False, year = None):
    '''
    Compares regions/countries
    "comp_dict" = dictionary in form {'regions': [], 'countries': []}
    If "plot_all_years" == True, plots cases over all years
    Otherwise, you must specify "year" to plot
    '''
